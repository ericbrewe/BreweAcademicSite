...
Day #2 is going to build on the results of yesterday, but we are actually going to analyze the network that we built in workshop #1. Also, I expect there will be a good deal more time available for discussion and questions. We will start with the network we made with attributes. We don't need to build the nework from scratch, instead I took the network we had and stored it as a .RDS file. RDS is the native structure for R.
