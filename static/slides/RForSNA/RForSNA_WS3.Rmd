...
- **Null hypothesis:** Exponential random graph models (ERGMs)
...
### If we want to estimate the confidence with which we think our measured netowk is different on one of these metrics, we can look for the percentiles.

Let's see if the measured average distance falls within the 95% of the simulated network....

```{r Quantile}

quantile(df$Dist, probs = c(0.05, 0.95))
```
